...
### 6.3. Governança e Autogovernança Dinâmica On-Chain de Validadores
A evolução da Jamii Blockchain para um modelo federado com governança determinística eliminou a troca de chaves públicas de validadores por vias dinâmicas inseguras ou voláteis, migrando para um sistema on-chain descentralizado e resiliente.
...
*   **Contrato de Autogovernança (`ValidatorRegistry`):** A adição ou exclusão de membros do comitê de validadores é gerenciada de forma descentralizada por um contrato inteligente na EVM. O modelo original de proprietário único (`owner`) foi substituído por uma mecânica de propostas (`propose`) e votação coletiva (`vote`). Apenas os validadores ativos podem interagir com a governança. A execução ocorre automaticamente assim que a proposta atinge maioria simples ($> N/2$) de aprovação.
