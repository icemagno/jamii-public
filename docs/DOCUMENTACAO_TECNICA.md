...
### 6.3. Governança, Staking e Slashing On-Chain de Validadores
A evolução da Jamii Blockchain para um modelo federado com governança determinística e incentivos econômicos eliminou a troca de chaves públicas de validadores por vias dinâmicas inseguras, consolidando o controle de admissão de validadores de forma on-chain e autogovernada na EVM.
...
*   **Contrato de Autogovernança (`ValidatorRegistry`):** A adição ou exclusão de membros do comitê de validadores é gerenciada de forma descentralizada por um contrato inteligente na EVM. O comitê de consenso ativo cria propostas (`propose`) e registra votos (`vote`) de forma colegiada, executando as alterações no conjunto de validadores automaticamente ao atingir o quórum de maioria simples ($> N/2$).
*   **Mecanismo de Depósito de Garantia (Staking):** Para evitar candidatos fantasmas e validar o comprometimento financeiro, o contrato exige que qualquer candidato a validador realize um depósito de fiança (`MIN_STAKE = 100.000 tokens`) usando a função `deposit()`. A função `propose()` exige a presença desse stake garantidor como pré-requisito obrigatório para submissão de inclusão de novos membros. Validadores removidos de forma honesta ou candidatos rejeitados podem resgatar seus fundos livremente via `withdraw()`.
*   **Regra de Slashing e Tesouraria no Gênese:** Caso um validador ativo seja removido da rede por votação do comitê (Slashing por má conduta, dupla assinatura ou inatividade severa), sua garantia sob fiança no contrato é confiscada e transferida integralmente para a conta de **Tesouraria** configurada. O endereço de destino da tesouraria é definido pela flag `"treasury"` no arquivo `genesis.json` e pré-alocado no **Slot 4** de armazenamento permanente do contrato inteligente durante a materialização do bloco gênese #0.
*   **Regra de Segurança Econômica (Custeio de Gas com Lucro de Validação):** O gas consumido pelas transações de proposta e voto deve ser obrigatoriamente pago pelo saldo acumulado de recompensas de mineração/validação de cada nó. Essa dependência econômica vincula diretamente a voz política do validador à sua alta disponibilidade e boa conduta:
  - Nós estáveis e ativos acumulam recompensas continuamente e possuem recursos suficientes para transacionar propostas e votos.
  - Nós inativos, instáveis ou intencionalmente maliciosos que fiquem fora da rede não recebem recompensas e, consequentemente, perdem a capacidade de votar nas propostas de governança por falta de gas.
  - Isso serve como um desincentivo natural a ataques de spam de propostas no comitê de consenso.
