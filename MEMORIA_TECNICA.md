...
- **Concluído:** Unified Identity, **Sovereign Transaction V1 (EIP-1559 Native)**, Mirroring nativo de saldo, Dívida Técnica Bloqueante (Gas Price, ReceiptsRoot, Buy Gas, VM Integration, JSON-RPC Read-only), Rede P2P (DTS Engine), MemPool (Gestão de transações pendentes com Purga Descendente), Sincronismo Determinístico (Besu-style), Conformidade Industrial IBFT2 (Pacing, Configurable Timeouts), Segurança Sync-to-Consensus (Observer Mode), Saneamento de Logs (Critical Level), Ghost Root Killer (Determinismo de Blocos Vazios), Resiliência de Sincronia (Channel Drainage), Compactação Soberana (Storage Optimization), Tsunami PQC Test (10.000 TXs Sustentadas), **Blindagem de Mercado (London/Besu Logic)**, **Otimização de Cache Industrial (MaxWarmTries Guard)**, **Sincronização Atômica Sync-Consensus (Anti-Self-Sabotage)**, **Chained State Oracle (Active Speculation)**, **Desacoplamento do SDK Java (Pure SDK)**, **Wallet Web App de Exemplo (Interativa)**, **Terminologia de Estado (Verkle/SMT)**, **Execução de Nós em Modo 100% Stateless (RAM Storage)**, **Validação Acelerada por Witness (Fase 7 & 9)**, **Muting de Consenso para Observadores**, **Reset Offline da MemPool (Otimização de Heaps)**, **DOM Reconciliation no Explorer**, **Mapeamento de Variáveis Docker e Teste de Conexão no Startup**, **Sincronização de BaseFee no Startup do Nó**, **Correção do Nonce Drift no Consenso via Pre-State Witness**, **Desativação Permanente do Paralelismo de Especulação**, **Keep-Alive Ativo de Status no Watchdog de Quórum**, **Filtro de Maturação de Transações (Propagation Delay Guard)**, **Empacotamento de Transações (Transaction Transfer Packet)**, **Purga Autônoma de Hot Nonces na MemPool**, **Sincronização Segura do Motor de Consenso (Prevenção de Stall em Sync Clássico)**, **Filtro de Quórum em Votações de RoundChange (IBFT 2.0)**, **Arquitetura de Poda em Disco para Nó Core (Sprints 1 a 3 em PRUNE.MD)**, **Janela de Descoberta Passiva Fixa no Boot**, **Restauração da Maturação da MemPool em Bloco Subsequente**, **Recuperação Reativa de Payloads no Consenso (tryFinalizeLocked)**, **Telemetria de Falhas Bi-Polar no Grafana**, **Restauração do Debounce de Sync e Tolerância do Propositor**, **Dynamic Cadence Throttle (DCT - Empacotamento Adaptativo por Degrau)** e **Agilidade Criptográfica (Signature Agility & Parametrização PQC).**


## 🛠️ Decisões Recentes (21/08/2026)
1. **Agilidade Criptográfica (*Signature Agility*) & Seleção PQC Parametrizada (`pkg/crypto/signer/hybrid.go`, `pkg/node/identity.go`, `pkg/wallet/wallet.go`, `pkg/core/genesis.go`, `CRYPTO-AGILITY.MD`):**
    - **Contexto:** Garantir que a Jamii Blockchain possua agilidade criptográfica total para adicionar ou substituir algoritmos pós-quânticos (ex: Falcon-512 ou SLH-DSA/SPHINCS+) sem quebrar transações, carteiras, o consenso IBFT ou o formato dos endereços (`jamii1...`).
    - **Ação 1 (Flexibilização dos Loaders Híbridos):** Refatorados os métodos `LoadHybridPublicKey` e `LoadHybridPrivateKey` em [`pkg/crypto/signer/hybrid.go`](file:///C:/Magno/Projetos/jamii/pkg/crypto/signer/hybrid.go#L294) substituindo a verificação hardcoded de `MLDSA65` pela função auxiliar extensível `isQuantumAlgorithm(algo)`.
    - **Ação 2 (Parametrização na Geração de Nós):** Adicionado o método `LoadOrCreateNodeKeyWithAlgo(dataDir, keyFileName string, algo signer.Algorithm)` em [`pkg/node/identity.go`](file:///C:/Magno/Projetos/jamii/pkg/node/identity.go#L30) permitindo passar o algoritmo PQC desejado (ex: `MLDSA65`, `Falcon512`), mantendo `LoadOrCreateNodeKey` como wrapper default.
    - **Ação 3 (Parametrização na Carteira / SDK):** Adicionado o método `FromMnemonicWithAlgo(mnemonic []byte, password string, algo signer.Algorithm)` em [`pkg/wallet/wallet.go`](file:///C:/Magno/Projetos/jamii/pkg/wallet/wallet.go#L89) para permitir que dApps e produtos escolham o algoritmo PQC em tempo real na reconstrução de carteiras BIP-39.
    - **Ação 4 (Especificação no Genesis & ChainConfig):** Adicionada a propriedade opcional `defaultQuantumAlgo` na struct `ChainConfig` em [`pkg/params/config.go`](file:///C:/Magno/Projetos/jamii/pkg/params/config.go) e documentada em [`docs/GENESIS.md`](file:///C:/Magno/Projetos/jamii/docs/GENESIS.md). Se omitida, assume graciosamente `MLDSA65` por padrão.
    - **Ação 5 (Telemetria de Algoritmo em Tempo Real):** Adicionados logs em vermelho claro (`\033[91m`) na inicialização do Gênesis ([`pkg/core/genesis.go`](file:///C:/Magno/Projetos/jamii/pkg/core/genesis.go#L162)) e durante a verificação do Handshake P2P no motor DTS ([`pkg/dts/engine.go`](file:///C:/Magno/Projetos/jamii/pkg/dts/engine.go#L931)) informando a identidade e algoritmo da chave pública verificada.
    - **Ação 6 (Documentação & Parecer Técnico Master):** Elaborado o parecer técnico oficial em [`CRYPTO-AGILITY.MD`](file:///C:/Magno/Projetos/jamii/CRYPTO-AGILITY.MD) e atualizada a documentação de arquitetura em [`docs/DOCUMENTACAO_TECNICA.md`](file:///C:/Magno/Projetos/jamii/docs/DOCUMENTACAO_TECNICA.md#L53) mapeando o Domínio Criptográfico (`0x00` a `0x06`).


## 🛠️ Decisões Recentes (20/08/2026)
1. **Dynamic Cadence Throttle - DCT (`pkg/consensus/ibft/controller.go`, `pkg/params/config.go`, `pkg/node/async_write_worker.go`):**
    - **Contexto:** Sob altas taxas de tráfego de transações pós-quânticas (picos de 300+ a 646 TPS), o tempo de bloco apresentava elevações pontuais devido à carga fixa de montagem e latências de disco. Além disso, a métrica de fila do `AsyncWriteWorker` apresentava leitura zerada no Grafana por amostragem do canal Go em nanograndezas.
    - **Ação 1 (Parametrização Genesis):** Adicionados os campos `maxTxsPerBlock` (default 2000), `minTxsPerBlock` (default 300) e `txStepSize` (default 50) à struct `ChainConfig` em [`pkg/params/config.go`](file:///c:/Magno/Projetos/jamii/pkg/params/config.go) e parametrizados nos arquivos `genesis.json`.
    - **Ação 2 (Algoritmo Dynamic Cadence Throttle - DCT):** Implementado no propositor IBFT ([`pkg/consensus/ibft/controller.go`](file:///c:/Magno/Projetos/jamii/pkg/consensus/ibft/controller.go)) o ajuste adaptativo do limite de extração da MemPool. Se a rodada anterior fechou rápido ($< 3{,}5\text{s}$) e há pendências, a capacidade expande em degrau (`+50 TXs`); se demorou ($> 6{,}0\text{s}$), contrai suavemente em degrau (`-50 TXs`), mantendo o tempo entre blocos cravado e imune ao efeito sanfona.
    - **Ação 3 (Ajuste Fino de Telemetria da Fila RAM):** Atualizado o método `QueueLength()` em [`pkg/node/async_write_worker.go`](file:///c:/Magno/Projetos/jamii/pkg/node/async_write_worker.go) para retornar `len(w.pendingMap)` com trava `RLock`, exibindo no Grafana a quantidade real de blocos retidos na RAM durante o descarte em disco.
    - **Ação 4 (Novo Painel no Grafana):** Adicionado o painel Timeseries `📦 Fila do Cache de Gravação Assíncrona (RAM) por Nó` em [`Grafana Dashboard/Grafana.json`](file:///c:/Magno/Projetos/jamii/Grafana%20Dashboard/Grafana.json).
2. **Janela de Descoberta Passiva Fixa no Boot (`pkg/node/node.go`):**
    - **Contexto:** Durante a fase de inicialização (*Discovery Phase*), a presença de um único peer de entrada (como um nó observador) fazia o loop de escuta passiva abortar antecipadamente aos 500ms (`len(ConnectedPeersIDs) > 0`). O loop subsequente de aguardo de discagem de saída abortava no milissegundo 0 porque aquele único peer preenchia a trava, impedindo que o discador de saída completasse as conexões dual-channel com os demais validadores da rede (que discam a cada 2s).
    - **Ação:** Reescrito o loop de escuta passiva em [`pkg/node/node.go`](file:///c:/Magno/Projetos/jamii/pkg/node/node.go) para garantir um tempo de escuta de **5 segundos fixos** (ou até que todos os validadores da genesis estejam conectados), dando tempo de sobra para a malha de validadores conectar e eliminando o encerramento precoce por peers isolados.
3. **Restauração da Maturação da MemPool em Bloco Subsequente (`pkg/mempool/pool.go`):**
    - **Contexto:** Em `GetExecutable`, a trava de maturação de transações usava a condição `>=` (`pt.addedAtHeight >= tp.currentHeight`). Quando o bloco $H$ era commitado, o `currentHeight` da MemPool virava $H$. Ao montar a proposta para o bloco $H+1$, todas as transações ingressadas durante a altura $H$ avaliavam `H >= H` como verdadeiro, sendo ignoradas e fazendo o propositor emitir blocos vazios (0 TXs) mesmo com a MemPool cheia.
    - **Ação:** Alterada a condição de maturação em [`pkg/mempool/pool.go`](file:///c:/Magno/Projetos/jamii/pkg/mempool/pool.go) para `pt.addedAtHeight > tp.currentHeight` (estritamente maior), permitindo que transações ingressadas na altura $H$ integrem normalmente a montagem do bloco $H+1$.
4. **Recuperação Reativa de Payloads sem Destruição de Rodada (`pkg/consensus/ibft/controller.go`):**
    - **Contexto:** Quando o quórum de commit era atingido via rede mas o corpo do bloco não estava pronto no motor de execução (por falha de reconstrução de bloco compacto), o `Controller` destruía a rodada ativa (`c.activeRound = nil`) e forçava uma parada para `Falling back to Sync`. Quando o corpo do bloco chegava via DTS poucas centenas de milissegundos depois, a rodada já havia sido zerada.
    - **Ação:** Implementado o método thread-safe `tryFinalizeLocked()` em [`pkg/consensus/ibft/controller.go`](file:///c:/Magno/Projetos/jamii/pkg/consensus/ibft/controller.go). Se o quórum for atingido mas o corpo do bloco ainda não estiver no motor de execução, a rodada permanece ativa e o dado é solicitado via DTS. Ao chegar a notificação `OnPayloadArrival`, o `tryFinalizeLocked()` é re-executado, selando e finalizando o bloco sem derrubar a rodada de consenso.
4. **Telemetria de Falhas de Reconstrução Bi-Polar no Grafana/Prometheus (`pkg/consensus/ibft/round.go`, `types.go` e `controller.go`):**
    - **Contexto:** As falhas de reconstrução de blocos compactos Bi-Polar eram registradas apenas no log do terminal (`log.Warn`), mas omitiam a chamada do gravador de métricas do nó (`RecordBipolarFailure`), fazendo com que os painéis do Grafana exibissem 0 falhas.
    - **Ação:** Adicionado o método `RecordBipolarFailure(missingCount uint64)` à interface `Transmitter` em [`pkg/consensus/ibft/types.go`](file:///c:/Magno/Projetos/jamii/pkg/consensus/ibft/types.go) e conectada a emissão de métrica na rotina `HandleProposal` em [`pkg/consensus/ibft/round.go`](file:///c:/Magno/Projetos/jamii/pkg/consensus/ibft/round.go). Toda falha de reconstrução em propostas IBFT agora incrementa imediatamente os contadores do Prometheus (`jamii_bipolar_reconstruction_failures_total` e `jamii_bipolar_missing_transactions_total`).
5. **Restauração da Tolerância de Sync e Propositor (`pkg/blockchain/sync.go` e `pkg/node/node.go`):**
    - **Contexto:** A trava rígida de 5 blocos (`targetHeight > currentHeight + 5`) desativou o mecanismo de **debounce de 500ms** para diferenças de 1 bloco (`targetHeight == currentHeight + 1`), fazendo com que pequenos atrasos de I/O de disco acumulassem até 6 blocos para disparar paradas abruptas de consenso (`HALT`). Além disso, a remoção da tolerância em `IsProposerReady` fazia propostas serem rejeitadas prematuramente.
    - **Ação:** (1) Restaurada a verificação `targetHeight > currentHeight` em [`pkg/blockchain/sync.go`](file:///c:/Magno/Projetos/jamii/pkg/blockchain/sync.go), reativando o debounce de 500ms para 1 bloco de diferença e ajustando a tolerância de buffer para 2 blocos (`currentHeight + 2 >= maxHeight`). (2) Restaurada em [`pkg/node/node.go`](file:///c:/Magno/Projetos/jamii/pkg/node/node.go) a tolerância de altura no `IsProposerReady` (`peerHeight >= currentHeight - 1`).
6. **Validação de Rede Estrita Estilo MetaMask / Trust Wallet na Jamii Wallet (`jamii-wallet/App.js`):**
    - **Contexto:** Necessidade de garantir que a carteira mobile Android (`jamii-wallet`) valide estritamente o endpoint RPC informado (`rpcUrl`) e o identificador de rede (`expectedChainId`) sem aceitar conexões com redes divergentes ou exibir dados desatualizados.
    - **Ação:** (1) Implementada a validação direta da chamada JSON-RPC `eth_chainId` no endpoint RPC fornecido na tela de Configurações, comparando estritamente o Chain ID retornado pela rede (ex: `2026` / `0x7ea`) com o Chain ID configurado (`expectedChainId`). (2) Aprovada a conexão (`chainIdStatus = 'valid'`) somente se o ID da rede for rigorosamente igual ao informado, atualizando o saldo silenciosamente via `eth_getBalance`. (3) Rejeitada a conexão com alerta visual (`chainIdStatus = 'mismatch'`) e zeramento preventivo de saldo em tela caso os Chain IDs divirjam. (4) Removido o alerta de pop-up em cases de validação bem-sucedida para garantir navegação fluida.
7. **Isolamento de Polling e Prevenção de Requisições por Caractere (`jamii-wallet/App.js`):**
    - **Contexto:** As variáveis de estado `rpcUrl` e `expectedChainId` na lista de dependências do `useEffect` de polling em segundo plano faziam com que o aplicativo disparasse requisições HTTP a cada caractere digitado na tela de Configurações, gerando erros parciais de rede com URLs incompletas.
    - **Ação:** (1) Removidas as variáveis `rpcUrl` e `expectedChainId` do array de dependências do `useEffect` de polling de segundo plano. (2) Restringida a execução do timer de polling (`setInterval(fetchNetworkData, 5000)`) exclusivamente às telas operacionais (`wallet` e `send`), impedindo a execução de chamadas de rede parciais durante a digitação de dados de configuração.
8. **Leitura Instantânea do Nonce em Tempo Real Pré-Assinatura (`jamii-wallet/App.js`):**
    - **Contexto:** Garantir que o envio de transações pela carteira utilize sempre o Nonce mais recente do nó RPC no exato momento da transmissão, prevenindo rejeições por nonce defasado.
    - **Ação:** Refatorada a função `handleSendTx` para realizar uma chamada RPC assíncrona instantânea (`eth_getTransactionCount` com o parâmetro `'pending'`) diretamente ao nó no exato milissegundo em que o usuário confirma o envio da transação, utilizando o nonce obtido em tempo real para a transmissão.
9. **Desativação do Android Auto Backup e Limpeza de Armazenamento Local (`jamii-wallet/app.json` e `App.js`):**
    - **Contexto:** O recurso nativo *Android Auto Backup* restaurava os dados legados do `AsyncStorage` da nuvem Google ao desinstalar e reinstalar o APK no aparelho, mantendo identificadores antigos (como `1337`).
    - **Ação:** (1) Adicionada a propriedade `"allowBackup": false` em `jamii-wallet/app.json` sob a chave `"android"`, instruindo o sistema operacional a desativar a restauração automática da nuvem. (2) Implementada a rotina de migração no `loadSavedWallet()` em `App.js` para atualizar automaticamente qualquer entrada antiga `1337` para `2026`.

## 🛠️ Decisões Recentes (13/08/2026)
1. **Guia Oficial de Configuração Operacional (`CONFIG.md`):**
    - **Contexto:** Necessidade de documentar exaustivamente todas as opções operacionais do arquivo `config.yaml`, incluindo parâmetros padrão carregados em código quando omitidos.
    - **Ação:** (1) Criado o documento [`CONFIG.md`](file:///c:/Magno/Projetos/jamii/CONFIG.md) detalhando as 18 opções do struct `NodeConfig` em [`pkg/node/config.go`](file:///c:/Magno/Projetos/jamii/pkg/node/config.go), explicando o funcionamento do Pruning, Nós Arquivadores (`is_archive_node`), Nós Stateless (`stateless`) e o mandato de Endereçamento Soberano Bech32 (`jamii1...`). (2) Atualizado o [`README.md`](file:///c:/Magno/Projetos/jamii/README.md) vinculando o novo guia.
2. **Saneamento da Coleta de Métricas de Tráfego DTS por Canal (`pkg/dts/engine.go`):**
    - **Contexto:** Conexões P2P antigas ou substituídas durante a fase de boot continuavam contabilizando bytes residuais nas variáveis globais `expressBytesOut` e `expressBytesIn`, gerando discrepâncias visuais nos gráficos do Grafana.
    - **Ação:** (1) Implementado o método helper thread-safe `isPeerActive(p *Peer) bool` em [`pkg/dts/engine.go`](file:///c:/Magno/Projetos/jamii/pkg/dts/engine.go) para checar se uma instância de `Peer` é rigorosamente a conexão ativa registrada no mapa `activePeers`. (2) Atualizadas as goroutines `writeLoop()` e `handleConnection()` para contabilizar tráfego apenas quando a conexão for a ativa. (3) Adotado o encerramento idempotente `SafeClose()` na substituição de sockets no boot.
3. **Implementação e Integração do Worker de Gravação Assíncrono - *Async Write Worker* (`ASYNC_WRITE_WORKER.md` e `pkg/node`):**
    - **Contexto:** O commit síncrono no PebbleDB (`AddBlockWithReceipts`) durante a execução do callback `OnBlockFinalized` levava entre $500\text{ ms}$ e $763\text{ ms}$, atuando como o gargalo físico para o avanço das rodadas de consenso.
    - **Ação:** (1) Criado o componente thread-safe `AsyncWriteWorker` em [`pkg/node/async_write_worker.go`](file:///c:/Magno/Projetos/jamii/pkg/node/async_write_worker.go) com fila buferizada em RAM, controle de congestionamento (*backpressure* com limite `maxQueueSize = 32`), buscas em RAM (`GetPendingBlockByNumber`/`GetPendingBlockByHash`) e flush gracioso no encerramento (`Stop()`). (2) Refatorado o callback `OnBlockFinalized` em [`pkg/node/node.go`](file:///c:/Magno/Projetos/jamii/pkg/node/node.go) para enfileirar as tarefas de gravação assíncrona, permitindo que a transição de altura de consenso ocorra em tempo de RAM ($< 5\text{ ms}$). (3) Adicionada a guarda de *Catch-Up Sync Protection* desativando a gravação assíncrona durante a sincronização histórica inicial. (4) Atualizado o manipulador P2P `handleBlockRequest` com busca fallback em 3 níveis (Disco -> Fila de RAM do AsyncWorker -> PayloadPool). (5) Exportada a métrica Prometheus `jamii_async_worker_queue_length` em [`pkg/rpc/server.go`](file:///c:/Magno/Projetos/jamii/pkg/rpc/server.go).
4. **Consolidação de Resiliência do Consenso IBFT 2.0 e Handover Buffer Dinâmico (`REPAROS-12-08.md` Sprints 1 a 3):**
...
5. **Descarte de Timeouts Obsoletos de Rodadas Commitadas (`pkg/consensus/ibft/round.go`):**
...
6. **Limites Explícitos de Prefixo em `CompactRange` (`pkg/store/prefixed.go`):**
...
7. **Telemetria de Tempo entre Blocos no Endpoint `/metrics` (`pkg/rpc/server.go` e `pkg/node/node.go`):**
...
8. **Expansão do Dashboard Grafana com Métricas de DB e Tráfego DTS por Canal (`Grafana Dashboard/Grafana.json`):**
...
## 🛠️ Decisões Recentes (29/07/2026)
